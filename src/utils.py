...
def visualize_with_RGB(image, keypoints, threshold=0.3, alpha=0.5):
    """
    ...
    for i in range(len(keypoints)):
        if keypoints[i, 2] > threshold:
            if KEYPOINT_INDEX_TO_NAME[i].startswith('left_'):
                image = cv2.circle(image, (int(keypoints[i][0]), int(
                    keypoints[i][1])), 2, blue_color, thickness=4)
            elif KEYPOINT_INDEX_TO_NAME[i].startswith('right_'):
                image = cv2.circle(image, (int(keypoints[i][0]), int(
                    keypoints[i][1])), 5, red_color, thickness=1, lineType=cv2.FILLED)
            else:  # nose
                image = cv2.circle(image, (int(keypoints[i][0]), int(
                    keypoints[i][1])), 5, green_color, thickness=1, lineType=cv2.FILLED)
